...
namespace app\components\deviceatlas;
...
class DeviceAtlasCloudClient {
    ...
    private static function getCloudService($userAgent, $cookie) {
        ...
        foreach ($servers as $i => $server) {
            $response = self::connectCloud(
                $server,
                'GET '. // don't HTTP/1.1
                    sprintf(self::CLOUD_PATH, urlencode(self::$licenseKey), urlencode($userAgent)).
                    " HTTP/1.0\r\n".
    ...
    private static function getServerLatency($server, $numRequests) {
        ...
        // common header parts
        $headers = 
            'GET '.sprintf(self::CLOUD_PATH, urlencode(self::$licenseKey), '')." HTTP/1.0\r\n".
            "Host: $server[host]:$server[port]\r\n".
            "Accept: application/json\r\n".
            "User-Agent: PHP/".self::API_VERSION."\r\n".
    ...
    protected static function __rankServers($servers=array()) {
        ...
        if (!$servers) {
            ...
            // sort by latency ASC
            usort($servers, array('app\components\deviceatlas\DeviceAtlasCloudClient', 'rankSort'));
        }
        // write to cache
